...
void main() async {
  ...
  try {
    ...
    if (dotenv.env['API_KEY'] == null || dotenv.env['API_KEY']!.isEmpty) {
      print("❌ API_KEY boş veya hatalı! Lütfen app.env dosyasını kontrol edin.");
    }

    if (dotenv.env['TODO_BASE_URL'] == null ||
        dotenv.env['TODO_BASE_URL']!.isEmpty) {
      print("❌ TODO_BASE_URL boş veya hatalı! Lütfen app.env dosyasını kontrol edin.");
    }

    if (dotenv.env['CALENDAR_API_BASE_URL'] == null ||
        dotenv.env['CALENDAR_API_BASE_URL']!.isEmpty) {
      print("❌ CALENDAR_API_BASE_URL boş veya hatalı! Lütfen app.env dosyasını kontrol edin.");
    }
  } catch (e) {
